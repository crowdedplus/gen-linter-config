...
(Note JavaTerm is terminology used in the Java, its format like [XXX],XXX is JavaTerm)
'''
dsl = '''RuleSet   ::= Rule1 ['And' | 'Or' | ';' Rule2]*  # 'And' means all Rule1 and Rule2 must be satisfied. 'Or' means allows any a single rule Rule1 or Rule2 to satisfy. ';' means Rule1 and Rule2 belong to different semantic groups. 

Rule   ::= ['Optional:' | 'Mandatory:'] [Constraint | Exception] # ExceptionRule represents exceptions where the rule does not apply.
 
Constraint   ::= TermList [Operator TermList]+ # TermList [Operator TermList]+ means the relationship of multiple TermList should satisfy
             | 'No' TermList [Operator TermList]+ # 'No' TermList means Prohibits TermList;
             | 'Order of' TermList ['is'|'is not'] TermList # 'Order of' means order rule of TermList is TermList
             | 'Order' ['is'|'is not'] TermList # 'Order' means order rule is TermList
             | 'Number of' TermList [Operator TermList]* #'Number' means numberConstraint
             | 'if' Rule1 'then' Rule2 # means Implied Relation Rule2 must be adhered to under the premise of Rule1 
...
Term   ::= '[' JavaScriptTerm ']' #Note: JavaScriptTerm represents terminology of JavaScript programming language; 
        | Modifier* Term 
        | Term 'of' Term 
...
Constraint   ::= TermList [Operator TermList]+ # TermList [Operator TermList]+ means the relationship of multiple TermList should satisfy
             | 'No' TermList [Operator TermList]+ 
             | 'Order of' TermList ['is'] TermList # 'Order of' means order of TermList is TermList
             | 'Order' ['is'] TermList # 'Order' means order among the elements of TermList
             | 'Number of' TermList [Operator TermList]* #'Number' means numberConstraint
             | TermList ['before'|'after'|'not before'|'not after' TermList]* | 描述一个实体前面或者后面必须存在/不存在TermList
             | TermList ['between'｜'not between' TermList]* | 描述TermList实体间存在/不存在TermList
             | TermList ['adjacent'｜'not adjacent' TermList]* | 描述TermList实体间需不需要相邻
...
Operator   ::= [.]+ (e.g., 'is' | 'is not' | 'have' | 'between' | 'after' |  '=' | 'Add ' | or others. '...' ) 
