"""Lightweight prompt for Checkstyle — uses JSON instead of DSL grammar.
...
from .Config_set_checkstyle_for_googlejava_ours_o1 import (
...
def step3_preprocess_promt(style="RuleSet of Google Java Style Guide", DSLruleset=None,
                           tool="Checkstyle", toolruleset=None, example=""):
...
2. From each matched rule's options list, find options whose "name" and "type" match what {{Style}} requires. Use the "default" value if {{Style}} does not specify a different value.
3. If the option "type" is a regular expression (Pattern), set the option value as a regular expression string. Otherwise, set a specific value from {{Style}}.
4. Identify the corresponding RuleName for each match and list all matched options.

**If there are no matching rules, the mappings should be None.**
Note: set specific option values should not be enclosed with '{}' or '[]', just a string is ok.

A rule in {{Style}} may correspond to multiple RuleNames of {{tool}}!
...
{{tool}} rules in JSON format. Each rule has "name", "desc", and "options" keys. Each option has "name", "type", "default", "description".
...
1. Optional: [ColumnLimit] is [100] not for [PackageStatement]
   >>>
   RuleName: LineLength
   option 1: ignorePattern; ^package\\s+.*;
   option 2: max; 100;
...
Constraint   ::= TermList [Operator TermList]+ # TermList [Operator TermList]+ means the relationship of multiple TermList should satisfy
             ...
             | TermList ['before'|'after'|'between'|'not before'|'not after'|'not between'|Operator TermList]*
             | TermList ['have'|'not have' TermList]*
             | TermList ['is'|'is not' TermList]*
...
def validation_config_superset_semantics_lightweight(mapping: str, style="RuleSet of Google Java Style Guide",
                                                     DSL_Syntax="", tool="Checkstyle",
